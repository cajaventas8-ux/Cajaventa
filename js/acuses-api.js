(function () {
  ...
  window.abrirImpresionDesdeModal = async function abrirImpresionDesdeModal() {
    ...
    if (!frame || !frame.contentWindow) {
      notify('No se pudo preparar la impresion del acuse.', 'error');
      return;
    }
  ...
  function renderPrintPreviewFrame(acuse) {
    const frame = document.getElementById('printPreviewFrame');
    if (!frame) return;
    ...
    if (!doc) return;
    doc.open();
    doc.write(buildPrintPreviewHtml(acuse));
    doc.close();
  }

  function buildPrintPreviewHtml(acuse = {}) {
    const detalles = Array.isArray(acuse.detalles) ? acuse.detalles : [];
    const detalleRows = detalles.length
      ? detalles.map((item, index) => `<tr><td>${index + 1}</td><td>${escapeHtml(item.Cod_Mercaderia || '')}</td><td>${escapeHtml(item.Descr_SAP || item.Cod_Mercaderia || '')}</td><td>${escapeHtml(formatQuantity(item.Cantidad))}</td><td>${escapeHtml(item.UM || '')}</td></tr>`).join('')
      : '<tr><td colspan="5" class="empty">Sin mercaderias cargadas.</td></tr>';
    const total = detalles.reduce((sum, item) => sum + Number(item.Cantidad || 0), 0);
    const cliente = acuse.Nom_Cliente || acuse.Cod_Cliente || 'Sin cliente';
    const nro = acuse.Nro_Acuse || acuse.ID_Acuse || '';

    return `<!doctype html>
<html lang="es">
<head>
<meta charset="utf-8">
<title>Acuse ${escapeHtml(nro)}</title>
<style>
  * { box-sizing: border-box; }
  body { margin: 0; padding: 28px; background: #eef2f7; color: #111827; font-family: Arial, sans-serif; }
  .sheet { max-width: 900px; margin: 0 auto; background: #fff; border: 1px solid #e5e7eb; border-radius: 14px; padding: 30px; box-shadow: 0 18px 40px rgba(15,23,42,.10); }
  .head { display: flex; align-items: center; justify-content: space-between; gap: 20px; border-bottom: 2px solid #0f62b9; padding-bottom: 18px; }
  .brand { display: flex; align-items: center; gap: 14px; }
  .brand img { width: 76px; max-height: 54px; object-fit: contain; }
  .brand h1 { margin: 0; font-size: 22px; color: #0f62b9; letter-spacing: 0; }
  .brand p, .meta p { margin: 4px 0 0; color: #6b7280; font-size: 12px; }
  .meta { text-align: right; }
  .meta strong { display: block; font-size: 18px; color: #111827; }
  .grid { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 12px 22px; margin: 24px 0; }
  .field { border: 1px solid #edf0f5; border-radius: 10px; padding: 10px 12px; background: #fbfdff; min-height: 58px; }
  .label { display: block; color: #6b7280; font-size: 10px; font-weight: 700; text-transform: uppercase; margin-bottom: 5px; }
  .value { font-size: 14px; font-weight: 700; color: #111827; overflow-wrap: anywhere; }
  table { width: 100%; border-collapse: collapse; margin-top: 14px; }
  th { background: #0f62b9; color: #fff; text-align: left; font-size: 11px; padding: 10px; text-transform: uppercase; }
  ...
  .total { margin-top: 16px; text-align: right; font-size: 14px; font-weight: 800; }
  .obs { margin-top: 18px; border: 1px dashed #cbd5e1; border-radius: 10px; padding: 12px; min-height: 60px; }
  .signatures { display: grid; grid-template-columns: 1fr 1fr; gap: 56px; margin-top: 56px; }
  .signature { border-top: 1px solid #111827; text-align: center; padding-top: 8px; color: #374151; font-size: 12px; }
  @media print { body { background: #fff; padding: 0; } .sheet { border: 0; border-radius: 0; box-shadow: none; max-width: none; } }
</style>
</head>
<body>
  <main class="sheet">
    <section class="head">
      <div class="brand"><img src="/assets/img/alas_logo.png" alt="ALAS"><div><h1>Acuse de entrega</h1><p>Caja Ventas</p></div></div>
      <div class="meta"><strong>${escapeHtml(nro || '--')}</strong><p>${escapeHtml(formatPreviewDate(acuse.Fecha_Emision) || '--')}</p></div>
    </section>
    <section class="grid">
      <div class="field"><span class="label">Cliente</span><span class="value">${escapeHtml(cliente)}</span></div>
      <div class="field"><span class="label">RUC</span><span class="value">${escapeHtml(acuse.Ruc_Cliente || '--')}</span></div>
      <div class="field"><span class="label">Direccion</span><span class="value">${escapeHtml(acuse.Direc_Cliente || '--')}</span></div>
      <div class="field"><span class="label">Ciudad / Zona</span><span class="value">${escapeHtml(acuse.Ciudad_Cliente || acuse.Zona || acuse.Zona_Cliente || '--')}</span></div>
      <div class="field"><span class="label">Repartidor</span><span class="value">${escapeHtml(acuse.Nombre_Repartidor || '--')}</span></div>
      <div class="field"><span class="label">Estado</span><span class="value">${escapeHtml(formatEstado(acuse.Estado))}</span></div>
    </section>
    <table><thead><tr><th>#</th><th>Codigo</th><th>Mercaderia</th><th>Cantidad</th><th>UM</th></tr></thead><tbody>${detalleRows}</tbody></table>
    <div class="total">Total unidades: ${escapeHtml(formatQuantity(total))}</div>
    <div class="obs"><span class="label">Observacion</span><span class="value">${escapeHtml(acuse.Observacion || '--')}</span></div>
    <section class="signatures"><div class="signature">Firma repartidor</div><div class="signature">Firma cliente</div></section>
  </main>
</body>
</html>`;
  }
